...
FROM ${BASE_IMAGE:?required} AS ngx_http_auth_jwt_builder_base
...
RUN <<`
	...
	apt-get update
	apt-get install -y \
		curl build-essential libjwt-dev libjwt0 libjansson-dev libjansson4 \
		libpcre2-dev zlib1g-dev libpcre3-dev
`
