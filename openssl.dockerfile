...
RUN <<`
    set -ex
    SSL_VERSION_NO_DOTS=$(echo ${SSL_VERSION} | tr . _)

    # Support two different URL formats used by OpenSSL over time.
    curl --silent --location --fail -O https://github.com/openssl/openssl/releases/download/OpenSSL_${SSL_VERSION_NO_DOTS}/openssl-${SSL_VERSION}.tar.gz ||
      curl --silent --location --fail -O https://github.com/openssl/openssl/releases/download/openssl-${SSL_VERSION}/openssl-${SSL_VERSION}.tar.gz
